...
const SCORE = {
  foot:  {ppu:6,   min:1.5, bonus:10, bonusMax:2, limit:100},
  wheel: {ppu:2,   min:1.5, bonus:10, bonusMax:2, limit:100},
  ex:    {ppu:1,                                  limit:100},
        ...
        {!cat&&(
          <div style={{padding:"20px 24px 40px"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {CATS.map(function(c){
                var used=dc[c.id]||0;var full=used>=LIM;
                ...
                return (
                  <button key={c.id} onClick={function(){if(!full)setCat(c.id);}} className="tap"
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{fontSize:12,color:T.grey,marginTop:3}}>{c.sub}</div>
                    </div>
                    <div>
                      {c.id==="phone"
                        ?<span style={{fontSize:11,fontWeight:700,color:T.navy}}>{gpsCount>0?gpsCount+" wykryte":"Zsynchronizowane"}</span>
                        :full
                        ?<span style={{fontSize:11,color:T.red,fontWeight:600}}>Limit 100 pkt</span>
                        :<div style={{textAlign:"right"}}>
                          <span style={{fontSize:13,fontWeight:700,color:c.col}}>{used}</span>
                          <span style={{fontSize:11,color:T.grey}}>/100 pkt</span>
                        </div>
                      }
                    </div>
                  </button>
        ...
        {cat&&cat!=="phone"&&act&&(
          <div style={{padding:"20px 24px 44px"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              {(cat==="foot"||cat==="wheel")&&(
                <div>
                  <div style={{fontSize:12,fontWeight:600,color:T.grey,letterSpacing:0.8,marginBottom:10}}>DYSTANS — min. 1.5 km</div>
                  <Stp value={km} onChange={setKm} min={0.5} max={200} step={0.5} unit="km"/>
                  {km<1.5&&<p style={{fontSize:12,color:T.red,marginTop:8,fontWeight:500}}>Minimum 1.5 km wymagane do zdobycia punktów</p>}
                </div>
              )}
              <div>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    ...
                    <div style={{fontSize:12,color:"rgba(255,255,255,0.5)",marginTop:3}}>
                      {cat==="foot"?"6 pkt/km"+(ac.foot<2?" + bonus 10":""):cat==="wheel"?"2 pkt/km"+(ac.wheel<2?" + bonus 10":"")+(commute?" + dojazd 25":""):cat==="ex"?"MET "+(MET[act.metKey]||6)+" × "+mins+"min/10":""}
                      {pb>0?"  ·  +"+pb+" passa":""}{awarded<prev+pb?"  ·  limit dzienny 100":""}
                    </div>
                  </div>
                  <div style={{textAlign:"right"}}>
  ...
  var sections=[
    ...
    },
    {
      ...
      body:[
        ...
        "2. Dzienny limit punktów w każdej kategorii wynosi 100 pkt.",
      ...
      <div style={{padding:"20px 20px 0",display:"flex",flexDirection:"column",gap:14}}>
        {[
          {icon:"🏃",lbl:"Na nogach",col:T.blue,rows:[
            {l:"Punktacja",v:"6 pkt za 1 km"},
            {l:"Minimum",v:"1.5 km (poniżej = 0 pkt)"},
            {l:"Bonus dzienny",v:"+10 pkt za 1. i 2. aktywność dnia"},
            {l:"Przerwa",v:"Min. 30 min między aktywnościami"},
            {l:"Limit",v:"100 pkt / dzień"},
          ]},
          {icon:"🚴",lbl:"Na kołach",col:"#2E7D32",rows:[
            {l:"Punktacja",v:"2 pkt za 1 km"},
            {l:"Dojazd do pracy",v:"+25 pkt bonus za rower do pracy"},
            {l:"Bonus dzienny",v:"+10 pkt za 1. i 2. aktywność dnia"},
            {l:"Limit",v:"100 pkt / dzień"},
          ]},
          ...
          {icon:"🚶",lbl:"Kroki",col:T.navy,rows:[
            {l:"Punktacja",v:"1.6 pkt / 1 000 kroków"},
            {l:"Bonus",v:"+10 pkt za 7 500 kroków"},
